...
- `.gitignore` antecipa o uso do Git e cria um arquvo de instruções ao Git para ignorar no repositório alguns arquivos padrão criados automaticamente pelo R e RStudio (`.Rproj.user`, `.Rhistory`, etc.);
...
Para criar o arquivo README basta executa o comando abaixo:
...
Dessa forma, agora podemos testar a instalação do pacote `{ipca}` em qualquer computador usando o comando abaixo (se o repositório for público; em caso de repositório privado utilize o argumento `auth_token`). Note que você deve substituir o nome da conta de usuário e nome do repositório para corresponder às suas informações. Se você executar o comando abaixo sem substituir as informações você obterá um erro.
...
Uma forma mais robusta de testar o seu pacote está funcional é o uso de testes de integração contínua. Este tópico é mais complexo e avançado, mas em termos práticos pode ser facilmente implementado usando uma função do `{usethis}` para configurar um workflow no GitHub Actions.
